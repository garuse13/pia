...
document.addEventListener('DOMContentLoaded', () => {
  ...
  input.style = 'position:fixed;bottom:60px;left:320px;width:500px;padding:8px;background:#2f3136;color:white;border:none;border-radius:5px;';
  document.body.appendChild(input);

  input.addEventListener('keydown', (e) => {
    if (e.key === 'Enter') {
      if (input.value.trim() === '!df') {
        let count = 0;
        const messages = ["디엠 와!", "ㅋㅋㅋㅋ", "🔥 긴급!", "중요합니다", "확인 좀", "테스트 중"];
        const floodInterval = setInterval(() => {
          const msg = document.createElement('div');
          msg.textContent = `User#${1000 + count}: ${messages[Math.floor(Math.random() * messages.length)]}`;
          msg.style = 'padding:8px;margin:4px;background:#404249;border-radius:6px;color:white;';
          container.appendChild(msg);
          container.scrollTop = container.scrollHeight;
          count++;
          if (count >= 500) clearInterval(floodInterval);
        }, 15);
      }
    }
  });
